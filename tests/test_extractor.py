...
class TestEntityExtractor:
    ...
    def test_extract_person(self, extractor):
        ...
        result = extractor.extract("Barack Obama met with Google in New York")
        # spaCy should recognize Barack Obama as PERSON
        people_lower = [p.lower() for p in result.people]
        assert any("obama" in p for p in people_lower), f"Expected 'Barack Obama' in people, got: {result.people}"
    ...
    def test_extract_organization(self, extractor):
        ...
        assert any("google" in o for o in orgs_lower), f"Expected 'Google' in organizations, got: {result.organizations}"

    def test_extract_date(self, extractor):
        """Should extract date expressions."""
        result = extractor.extract("The meeting is on Monday, January 15th")
        assert len(result.dates) > 0, f"Expected at least one date, got: {result.dates}"
    ...
    def test_extract_location(self, extractor):
        ...
        assert any("new york" in loc for loc in locations_lower), f"Expected 'New York' in locations, got: {result.locations}"
    ...
    def test_deduplication(self, extractor):
        ...
        result = extractor.extract("Obama met Obama. Barack Obama said Obama was there.")
