...
("Vendor choice",
 "Why was Backblaze B2 chosen over AWS S3?"),

("Person lookup",
 "What is Priya responsible for and where is she based?"),

("Action items",
 "What are Rahul's current deadlines?"),

("Budget research",
 "What is our planned pricing for Project Phoenix?"),

("Email content",
 "What concern did Priya raise about the database schema?"),

("Competitor",
 "What are the weaknesses of FinanceTrack Pro?"),
...
"[bold]ContextOS — Sample Data Test[/bold]\n"
"Ingesting 5 realistic documents then asking 8 questions.\n"
"No Gmail or cloud needed.",
border_style="blue"
))
# Start engine
console.print("\n[yellow]Starting engine...[/yellow]")
engine = ContextEngine()

# Check Ollama
if not engine.is_ready():
    console.print("[red]Ollama not ready. Make sure the Ollama app is open.[/red]")
    return
...
for filename, content in SAMPLE_DOCUMENTS.items():
    count = engine.ingest_text(content, source=filename)
    total_chunks += count

console.print(f"\n[green]✓ Ingested {len(SAMPLE_DOCUMENTS)} documents → {total_chunks} chunks[/green]")
console.print(f"[dim]Vector store now has {engine.vector_store.count()} total chunks[/dim]\n")

# Run test questions
console.print("[yellow]Running test queries...[/yellow]\n")

results_table = Table(
    "Test", "Question", "Answer (truncated)", "Time",
...
console.print(Panel.fit(
    ...
    f"Vector store: {engine.vector_store.count()} chunks\n"
